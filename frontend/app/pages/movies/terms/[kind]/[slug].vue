  ...
  <main class="term-page">
    ...
    <template v-else-if="data">
        ...
        <component :is="spotlightWrapper" :to="spotlightMovie?.href || undefined" class="spotlight-link">
          <article class="spotlight-card">
            ...
            <div class="spotlight-body">
              ...
              <p v-else class="spotlight-note">Ainda não há um filme ativo para abrir esse recorte.</p>
            </div>
          </article>
        </component>
      </section>
      ...
      <MoviesLibraryGrid
        ...
        :items="data.movies"
        layout="aligned"
        empty-message="Nenhum filme ativo apareceu para esse termo."
      />
    </template>
  </main>
</template>
...
import { useMovieTermPageData } from '~/composables/useMovieTermPageData'
...
const { data, error, status } = await useMovieTermPageData(kind.value, slug.value)
...
const heroIntro = computed(() => {
  ...
  return data.value.kind === 'GENRE'
    ? 'Um corte da biblioteca por classificação ampla, para percorrer o catálogo sem perder a mesma lógica visual do arquivo.'
    : 'Um corte mais específico da biblioteca, reunindo os filmes que compartilham esta marcação dentro do catálogo.'
})
const gridTitle = computed(() => (data.value ? `Filmes com ${data.value.name}` : 'Os filmes deste termo'))
const gridDescription = computed(() =>
  data.value?.kind === 'GENRE'
    ? 'A mesma biblioteca, agora filtrada por um gênero específico.'
    : 'A mesma biblioteca, agora filtrada por uma marcação mais específica.',
)
...
useHead(() => ({
  title: data.value ? `${data.value.name} · Filmes · Media Pulse` : 'Termo · Filmes · Media Pulse',
  meta: [
    {
      name: 'description',
      content: data.value
        ? `Filmes ligados ao termo ${data.value.name} no Media Pulse.`
        : 'Página interna de termo de filme no Media Pulse.',
    },
  ],
}))
</script>
