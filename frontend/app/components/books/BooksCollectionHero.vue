<template>
  <section class="books-collection-hero" :style="heroShellStyle">
    <div class="top-actions">
      <NuxtLink class="back-link" to="/"> Voltar para a capa </NuxtLink>

      <NuxtLink class="section-link" to="/books?view=archive"> Abrir arquivo inteiro </NuxtLink>
    </div>
      ...
      <component :is="leadWrapper" :to="lead?.href || undefined" class="lead-link">
        <article class="lead-card">
          <div class="lead-cover">
            ...
            <div v-else class="lead-fallback">{{ (lead?.title || 'L').slice(0, 1) }}</div>
          </div>

          <div class="lead-body">
            <p class="lead-kicker">{{ lead?.eyebrow || 'Livro' }}</p>
            <h2>{{ lead?.title || 'Sem livro em destaque agora' }}</h2>
...
.lead-card {
  display: grid;
  grid-template-columns: minmax(12rem, 0.76fr) minmax(0, 1fr);
  gap: 20px;
