<template>
  <section class="music-collection-hero">
    <div class="top-actions">
      ...
      <NuxtLink class="section-link" to="/music/library">
      ...
      <component :is="leadWrapper" :to="lead?.href || undefined" class="lead-link">
        <article class="lead-card">
          <div class="lead-cover">
            ...
            <div v-else class="lead-fallback">{{ (lead?.title || 'M').slice(0, 1) }}</div>
          </div>

          <div class="lead-body">
            <p class="lead-kicker">{{ lead?.eyebrow || 'Música' }}</p>
            <h2>{{ lead?.title || 'Sem destaque musical agora' }}</h2>
