...
Durante a validação em container, `Special:EntityData/{id}.json` devolveu `400` em HTML exclusivamente para a requisição criada pelo WebClient, embora o mesmo endereço respondesse `200` via `curl` no host e no namespace de rede do container. A troca para `/w/api.php?action=wbgetentities&props=claims` revelou um `401` com `Jwt issuer is not configured`: o builder HTTP compartilhado havia herdado o token do TMDB/Hardcover e o enviava ao Wikidata. Os clientes agora clonam o builder antes de adicionar configuração própria, e o cliente Wikimedia remove defensivamente qualquer `Authorization`. A ação `wbgetentities` permanece por retornar somente os dados necessários e manter o contrato de `P18`.
