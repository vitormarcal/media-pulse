...
`PATCH /api/shows/lists/{listId}/rules` recebe `{"includeFavorites": true, "includeAbandoned": false}` e retorna `ShowListSummaryDto` (404 se a lista não existir). Ambos os campos são booleanos.

Resumos e detalhes retornam `includeFavorites` e `includeAbandoned`. O detalhe inclui `manualShowIds`, na ordem manual. Contagens, previews e itens consideram a união sem duplicatas entre inclusões manuais e obras que atendem a pelo menos uma regra. Desmarcar remove apenas a inclusão automática. A atualização aparece na próxima leitura.
