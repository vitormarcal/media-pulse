...
class ImportPipelineRunnerTest {
    ...
    @Test
    fun `should skip pipeline when disabled`() = runBlocking {
    ...
    @Test
    fun `should run all enabled services when pipeline is enabled`() = runBlocking {
        ...
        coEvery { plexImportService.importAllArtistsAndAlbums(pageSize = 200) } returns
            PlexImportService.ImportStats(artistsSeen = 10, artistsUpserted = 10, albumsSeen = 50, albumsUpserted = 50, tracksSeen = 500, tracksUpserted = 500)
    ...
    @Test
    fun `should skip plex import when disabled`() = runBlocking {
        coEvery { pipelineProps.enabled } returns true
